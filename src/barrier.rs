...
mod linux {
    ...
    /// Issues a light memory barrier for fast path.
    ///
    /// It issues a compiler fence, which disallows compiler optimizations
    /// across itself. It incurs basically no costs in run-time.
    #[inline]
    pub fn light() {
        use self::Strategy::*;
        match *STRATEGY {
    ...
    #[inline]
    pub fn light_ptr_store<T>(m: &atomic::AtomicPtr<T>, value: *mut T) {
        use self::Strategy::*;
        match *STRATEGY {
            Membarrier | Mprotect => {
    ...
    /// Issues a heavy memory barrier for slow path.
    ///
    /// It issues a private expedited membarrier using the `sys_membarrier()`
    /// system call, if supported; otherwise, it falls back to
    /// `mprotect()`-based process-wide memory barrier.
    #[inline]
    pub fn heavy() {
        use self::Strategy::*;
        match *STRATEGY {
...
mod windows {
    ...
    ///
    /// It issues compiler fence, which disallows compiler optimizations across
    /// itself.
    #[inline]
    pub fn light() {
        atomic::compiler_fence(atomic::Ordering::SeqCst);
